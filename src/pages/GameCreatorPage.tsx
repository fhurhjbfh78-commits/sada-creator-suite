...
import { Plus, Trash2, Play, Square, CircleDot, Star, Copy, Settings, Eye, EyeOff, Lock, Unlock, Download } from 'lucide-react';
...
type Tab = 'editor' | 'projects' | 'settings' | 'export' | 'layers';

const GameCreatorPage = () => {
  const {
    gameProjects, activeProjectId, createGameProject, deleteGameProject,
    setActiveProject, updateGridCell, updateProjectSettings, profile
  } = useAppStore();
  ...
  const handleCreateProject = () => {
    ...
    const id = createGameProject(newProjectName, newProjectSize);
    setActiveTab('editor');
    setNewProjectName('');
  };
        ...
        {tabs.map(({ id, label }) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all active:scale-95 ${
              activeTab === id ? 'glow-btn' : 'glass-card text-muted-foreground'
            }`}
          >
            {label}
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
          ...
          <div className="space-y-3 animate-fade-in">
            <div className="glass-card p-4">
              ...
              <input
                ...
                className="w-full glass-input px-3 py-2.5 text-sm text-right mb-2 text-foreground"
                placeholder="اسم المشروع"
              />
              <div className="flex items-center gap-2 mb-3 justify-end">
                <select
                  value={newProjectSize}
                  onChange={(e) => setNewProjectSize(Number(e.target.value))}
                  className="glass-input px-3 py-2 text-sm bg-secondary text-foreground"
                >
                  {[8, 10, 12, 16, 20].map(s => (
            ...
            {gameProjects.map((p) => (
              <div
                key={p.id}
                className={`glass-card p-4 flex items-center justify-between active:scale-[0.98] transition-transform cursor-pointer ${
                  p.id === activeProjectId ? 'border-primary/50' : ''
                }`}
                onClick={() => { setActiveProject(p.id); setActiveTab('editor'); }}
              >
                <button
                  onClick={(e) => { e.stopPropagation(); deleteGameProject(p.id); }}
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {p.gridSize}x{p.gridSize} · {new Date(p.updatedAt).toLocaleDateString('ar')}
                  </p>
                </div>
              </div>
            ))}
            ...
            {gameProjects.length === 0 && (
              <p className="text-center text-muted-foreground text-sm py-8">لا توجد مشاريع</p>
            )}
          </div>
        )}
          ...
          <div className="space-y-3 animate-fade-in">
            ...
            <div className="glass-card p-2 aspect-square">
              <div
                ...
                style={{
                  gridTemplateColumns: `repeat(${project.gridSize}, 1fr)`,
                  backgroundColor: project.showGrid ? 'hsl(210 30% 20%)' : 'transparent',
                }}
              >
                {project.grid.map((row, ri) =>
                  row.map((cell, ci) => (
                    <button
                      ...
                      className="aspect-square transition-colors active:scale-90"
                      style={{
                        backgroundColor: getCellColor(cell.element) || project.bgColor,
                        opacity: hoverCell?.row === ri && hoverCell?.col === ci ? 0.7 : 1,
                      }}
                    />
                  ))
                )}
            ...
            <div className="glass-card p-3 flex gap-2 overflow-x-auto">
              {TOOLS.map(({ id, icon: Icon, color, label }) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all active:scale-90 min-w-[60px] ${
                    selectedTool === id ? 'bg-primary/20 ring-1 ring-primary' : 'bg-secondary/50'
                  }`}
                >
            ...
            {/* Status bar */}
            <div className="glass-card px-3 py-2 flex items-center justify-between text-[10px] text-muted-foreground">
              <span>أداة: {TOOLS.find(t => t.id === selectedTool)?.label}</span>
              <span>{hoverCell ? `${hoverCell.col}, ${hoverCell.row}` : '—'}</span>
              <span>Developed by: عبدالله لازم</span>
            </div>
          </div>
        )}
        ...
        {activeTab === 'editor' && !project && (
          <div className="text-center py-12">
            ...
            <button onClick={() => setActiveTab('projects')} className="glow-btn px-6 py-2 mt-4 text-sm active:scale-95 transition-transform">
              المشاريع
            </button>
          </div>
        )}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <input
                    ...
                    onChange={(e) => updateProjectSettings(project.id, { bgColor: e.target.value })}
                ...
                <div className="flex items-center justify-between">
                  <button
                    onClick={() => updateProjectSettings(project.id, { showGrid: !project.showGrid })}
                    className={`px-4 py-2 rounded-lg text-xs ${project.showGrid ? 'glow-btn' : 'glass-card'}`}
                  >
                    {project.showGrid ? 'مفعل' : 'معطل'}
                  </button>
                ...
                <div className="flex items-center justify-between">
                  <select
                    ...
                    onChange={(e) => updateProjectSettings(project.id, { gridSize: Number(e.target.value) })}
                    className="glass-input px-3 py-2 text-sm bg-secondary text-foreground"
                  >
                    {[8, 10, 12, 16, 20].map(s => (
                      <option key={s} value={s}>{s}x{s}</option>
        ...
        {activeTab === 'export' && project && (
          <div className="space-y-3 animate-fade-in">
              ...
              {(['json', 'xml', 'text'] as const).map(f => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 ${
                    exportFormat === f ? 'glow-btn' : 'glass-card'
                  }`}
                >
                  {f.toUpperCase()}
                </button>
            ...
            <div className="glass-card p-3">
              <pre className="text-[10px] text-foreground overflow-auto max-h-[50vh] whitespace-pre-wrap font-mono" dir="ltr">
                {exportData}
              </pre>
            </div>
            <button
            ...
            {TOOLS.map(({ id, label, color }) => (
              <div key={id} className="glass-card p-3 flex items-center justify-between">
                <div className="flex gap-2">
                  <button
                    onClick={() => setLayerLocked(l => ({ ...l, [id]: !l[id] }))}
                  ...
                  <button
                    onClick={() => setLayerVisibility(v => ({ ...v, [id]: v[id] === false ? true : false }))}
                    className="p-1.5 rounded-lg hover:bg-secondary/50"
                  >
